...
class LoanRepository:
    ...
    def get_by_user(self, user_id: UUID) -> list[Loan]:
        return self.db.query(Loan).filter(Loan.user_id == user_id).all()

    def get_all(self) -> list[Loan]:
        return self.db.query(Loan).all()

    def create(self, user_id: UUID, amount: float, duration_months: int, purpose: str | None = None) -> Loan:
        loan = Loan(user_id=user_id, amount=amount, duration_months=duration_months, purpose=purpose)
        self.db.add(loan)
        self.db.commit()
