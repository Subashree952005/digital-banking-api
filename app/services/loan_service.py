...
class LoanService:
    ...
    def apply(self, user_id: UUID, data: LoanApplyRequest):
        if data.amount <= 0:
    ...
    def get_loan(self, loan_id: UUID, user_id: UUID | None = None):
    ...
    def get_user_loans(self, user_id: UUID):
        return self.repo.get_by_user(user_id)

    def get_all_loans(self):
        return self.repo.get_all()

    def approve(self, loan_id: UUID, officer_id: UUID, data: LoanReviewRequest):
        loan = self.repo.get_by_id(loan_id)
        if not loan:
            raise HTTPException(status_code=404, detail="Loan not found")
    ...
    def reject(self, loan_id: UUID, officer_id: UUID, data: LoanReviewRequest):
        loan = self.repo.get_by_id(loan_id)
        if not loan:
            raise HTTPException(status_code=404, detail="Loan not found")
        if loan.status != LoanStatus.pending:
    ...
    def update_loan(self, loan_id: UUID, **kwargs):
        loan = self.repo.get_by_id(loan_id)
        if not loan:
            raise HTTPException(status_code=404, detail="Loan not found")
        return self.repo.update(loan, **kwargs)

    def delete_loan(self, loan_id: UUID):
        loan = self.repo.get_by_id(loan_id)
        if not loan:
            raise HTTPException(status_code=404, detail="Loan not found")
        self.repo.delete(loan)

    def calculate_emi(self, loan_id: UUID) -> EMIResponse:
        loan = self.repo.get_by_id(loan_id)
        if not loan:
            raise HTTPException(status_code=404, detail="Loan not found")
